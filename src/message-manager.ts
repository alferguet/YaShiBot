    ...
    async process(message: Discord.Message): Promise<void> {
        try {
            ...
            if (messageContent.startsWith('ysb!')) {
                ...
                switch(module){
                    case 'glossary':
                        await message.channel.send(this.glossaryManager.getDefinition(commandData))
                        break
                    case 'help':
                        await message.channel.send(`WIP`)
                        break
                    default:
                        await message.channel.send(`The request doesn't match any features of this bot`)
                        break
                }
            }
        } catch (err) {
